...
namespace BasketService.Business.Concrete
{
    public class BasketItemManager : IBasketItemService
    {
        ...
        public IResult RemoveBasketItem(int itemId)
        {
            try
            {
                BasketItem basketItem = new()
                ...
                var result = _basketItemDal.Get(x => x.BasketId == basketItem.Id);
                _basketItemDal.Delete(result);
                return new SuccessResult("Sebetden silindir");
